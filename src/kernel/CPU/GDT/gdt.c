...
volatile bool gdt_loaded = false;
...
void initGdt() {
    ...
    setGdtGate(1, 0, 0xFFFFF, 0x9A, 0xCF); // Kernel code segment
    setGdtGate(2, 0, 0xFFFFF, 0x92, 0xCF); // Kernel data segment
    setGdtGate(3, 0, 0xFFFFF, 0xFA, 0xCF); // User code segment
    setGdtGate(4, 0, 0xFFFFF, 0xF2, 0xCF); // User data segment
    writeTSS(5, 0x10, 0x0);

    if (gdt_ptr.limit == 0 || gdt_ptr.base == 0) {
        LOG_ERR("GDT setup failed, falling back");
        bare_cli();
        return;
    }
